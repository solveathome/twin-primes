...
verdict: For a prescribed fixed exponent margin eta0, the zero budgets leave S_0={p<2eta0,q<2eta0} below that required margin regardless of the nonzero-kernel saving. Uniform gamma=2 covers its complement at that prescribed margin. Smaller positive margins can reach fixed interior points of S_0; only p=q=0 remains at zero budget one for every margin. The proposed eta0~loglog x/log x substitution is not justified by estimates carrying x^epsilon losses; its box count is geometry only. The sufficient twin margin remains OPEN. Independent review F3 and F5 correct the quantifiers and the prime-power tail proof; round-review-0906 corrects the diagonal scope, fixed-margin bulk claim and six-cut separation.
-->
...
### 2.5 Fixed margins and the proposed shrinking corner

The analytic bounds currently require a fixed positive margin. Substituting
eta_0=C*loglog x/log x into the displayed exponents alone does not prove an
O_H(x/log^H x) estimate: the proofs also carry O_epsilon(x^epsilon) losses,
and for fixed epsilon>0, x^epsilon/(log x)^(cC) grows without bound. Nor
may epsilon vary with x without controlling its constants. A uniform
logarithmic-loss replacement has not been derived.

The following is therefore **geometric bookkeeping conditional on a new
uniform error bound**, not the analytic size of the proved remainder:
...
For fixed eta_0, (1) still proves failure to obtain that *prescribed*
margin on S_0. It does not exclude every smaller positive saving at its
interior points. With hypothetical gamma=2 and p=q=eta_0, for example,
the zero and cross budgets are 1-eta_0/2 and 1-2eta_0. The exact endpoint
p=q=0 retains zero budget 1. See
[independent review F3](history/reviews-0906/20-independent-handoff-review.md).
...
The qualitative consequence is the same in all three readings: a
triangle-inequality treatment of S_0 misses x/log^K x by at least a
factor log^K x (by log^(K+2) at fixed eta_0, by (loglog x)^2 log^K x at
eta_0 of order loglog x/log x, by log^K x on the single top box). This
bounds absolute term mass, both signs together, and says nothing about
the signed value. It is the S_0-local analogue of
[singleton-fiber-audit.md](singleton-fiber-audit.md)'s global c*x*log x
absolute-mass lower bound, without that note's per-sign split: a per-sign
bound on S_0 would need the sign of mu along the CRT progression, which is
the OPEN two-point correlation itself.
...
2. **One Cauchy inequality in the other divisor**, costing
   ...
   this is an upper bound under its general hypotheses. A uniform lower
   bound would require nondegeneracy of the actual coefficient and
   support; it is not supplied by those hypotheses. The region arithmetic
   uses the available upper-budget exponent a/2.
3. **Zero frequencies bounded absolutely.**
   ...
   at most a constant times B^2 C^2 f^2 MN/A. It can vanish for zero
   coefficients, empty harmonics or coincident endpoints. The upper
   budget reaches x on A ~ MN/x; that is not a uniform lower bound.

Keeping these upper budgets unchanged leaves the right block estimate
sqrt(M*x) = x^((1+a)/2), even if the nonzero-frequency budget improves.
This is a limitation of the displayed estimates, not a lower bound on
the actual sum or moment. A sharper coefficient norm, diagonal estimate
or different organization could change it; no necessary cancellation
mechanism is proved here. DERIVED, inside this budget calculation only.
...
s < x^(1-a) = x^p < x^(2*eta_0). Since k < V^2 for eta_0<3/25, at most
one prime base has powers above V dividing k; several powers of that same
base can occur. The proper-prime-power class is negligible because
sum_{P^j>V,j>=2} P^(-j) << V^(-1/2): split P at sqrt(V), bound each
small-base tail by 2/V, and sum 2/P^2 over the larger bases. Counting
multiples and divisor multiplicities costs O_epsilon(x^(22/25+epsilon)),
which is O_H(x/log^H x) when epsilon<3/25. After discarding this class,
r is prime and beta_V(k)=log r. DERIVED; this repairs the false inference
P^j>V implies P>sqrt(V) for every j>=2.
...
s' < x^(1-b) = x^q < x^(2*eta_0). The same prime-power tail bound with
W=Z costs O_epsilon(x^(39/40+epsilon)), which is O_H(x/log^H x) for
epsilon<1/40. This includes small bases with powers of exponent at least
three; no false lower bound on the prime base is used. Then
...
On the s=s'=1 subfamily of S_0 the determinant relation becomes d*r - e*r' = 2 with d~x^(19/25),
...
prime-cofactor subfamily is therefore an averaged binary Mobius correlation along two
...
The full corner also contains k=rs,t=r's' with s or s' greater than one,
including the non-squarefree and gcd-two branches. They have not been
discarded at an admissible error rate. The exact full object is (4) of
[corner-correlation.md](corner-correlation.md); its nonnegative
Möbius-product form (5) applies only to the subfamily above.
...
The row at 37/25 exceeds the row at 3/2 because 37/25<3/2, although
that row is printed later. At increasing gamma the sets are nested and
their areas are nonincreasing; clipping does not reverse monotonicity.
...
62.9 to 61.1. Second, in the limiting eta_0=0 geometry a bulk interior
persists for gamma<2 by taking small positive p=q<(2-gamma)/4.
At a prescribed positive eta_0, the bulk with p,q>=2eta_0 is empty
once gamma>=2-6eta_0. Edge strips still prevent full coverage outside
S_0 for gamma<2; see §4.5 and round-review-0906 §5.
...
Separation follows [residual-coverage §4](residual-coverage.md) with
up to six truncated Perron integrals: A and B each have one scalar
cutoff, C and D each have two. Use real part 1/log x, height x^10
and half-integer-shifted thresholds. In the order A,B,C_vertical,
C_monomial,D_vertical,D_monomial the combined twist is
d^(-s1-5s2-s3-s4-3s6) e^(-s1-2s2-3s4-s5-s6).
All real parts are nonnegative, so the endpoint bounds apply uniformly
in the imaginary parts; the cost is O(log^6 x) and the indicator error
...
then exactly S_0. For gamma<2 the point p=2eta_0,q=0 fails the right
cross inequality and the left zero inequality; the symmetric point
does likewise. Thus both edge strips minus S_0 persist, although the
bulk with p,q>=2eta_0 is already empty when gamma>=2-6eta_0.
The overall threshold remains gamma=2, not a lower-bound theorem for
the actual arithmetic remainder.
...
1. At a prescribed fixed margin eta_0, S_0 fails that margin for every
   gamma in both orientations, because the zero budget is unchanged.
   This does not exclude smaller margins at its fixed interior points.
2. gamma=2 is the threshold at which the leftover reduces to S_0;
   both edge strips minus S_0 persist for gamma<2. At fixed eta_0 the
   bulk is already empty for gamma>=2-6eta_0.
...
7. Choosing eta_0 of order loglog x/log x gives the stated geometric
   box count, but analytic coverage at that margin is unproved (§2.5).
8. Only the s=s'=1 subfamily reduces to (5). The full corner retains
   small-cofactor and non-squarefree branches. Proper-prime-power classes
   are negligible by the corrected tail bounds in §§3.1–3.2.
